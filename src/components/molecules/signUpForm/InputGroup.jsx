    ...
    return (
        <InputLayout label={label} error={error}>
            <input type={type} name={name} maxLength={maxLength}
                ...
                onFocus={() => (type !== 'number' ? handleFocus() : () => { })} value={val} readOnly={!isEditable} className="border-none placeholder:text-black/40 focus-visible:outline-0 p-3 text-center w-full" placeholder={placeholder} ref={InputRef} onChange={handleChange} />
        </InputLayout>
    )
}
)
